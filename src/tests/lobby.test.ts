...
describe('host', () => {
    ...
    test('should create a game than close it and cleanup properly', async () => {
        //Open server
        let prom = hostSocket.emit(SocketEvents.createGame, 'username');
        await waitFor(hostSocket, SocketEvents.createGame);
        ...
        //Close server
        prom = hostSocket.emit(SocketEvents.gameClosed, 'username');
        prom = await waitFor(hostSocket, SocketEvents.gameClosed);
...
describe('host and 4 clients', () => {
    ...
    test('should create a game join it than close it and cleanup properly', async () => {
        //Open server
        let prom = hostSocket.emit(SocketEvents.createGame, { username: 'username' });
        let ret = <{ gameId: string }>await waitFor(hostSocket, SocketEvents.createGame);

        clientSocket1.emit(SocketEvents.joinGame, { username: 'cl1', gameId: ret.gameId });
        clientSocket2.emit(SocketEvents.joinGame, { username: 'cl1', gameId: ret.gameId });
        clientSocket3.emit(SocketEvents.joinGame, { username: 'cl1', gameId: ret.gameId });
        clientSocket4.emit(SocketEvents.joinGame, { username: 'cl1', gameId: ret.gameId });

        await clientsWaitForEvent(SocketEvents.joinGame);
        let socks = await io.fetchSockets();
        ...
        //Close server
        prom = hostSocket.emit(SocketEvents.gameClosed, 'username');
        prom = await usersWaitForEvent(SocketEvents.gameClosed);
